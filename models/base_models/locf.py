...
from libs.pypots.imputation.base import BaseImputer
...
class LOCF(BaseImputer):
    ...
    def locf_torch(self, X, mask=None):
        ...
        if mask is None:
            ...
            mask = mask.gt(0)
        n_samples, n_steps, n_features = mask.shape
        idx = torch.where(~mask, torch.arange(
            n_features, device=mask.device), 0)
        idx = torch.cummax(idx, dim=2)

        collector = []
        cnt = 1
        for x, i in zip(trans_X, idx[1]):
            cnt += 1
