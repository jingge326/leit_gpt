...
class GRUObservationCellLogvar(torch.nn.Module):
    ...
    def __init__(self, input_size, hidden_size, prep_hidden, bias=True):
        ...
        self.w_prep = torch.nn.Parameter(
            std * torch.randn(input_size, 4, prep_hidden))
        self.bias_prep = torch.nn.Parameter(
...
class NNFOwithBayesianJumps(torch.nn.Module):
    ...
    def __init__(self, args):
        ...
        self.impute = impute
        self.solver = solver
        self.store_hist = False
        self.input_size = input_size
        self.logvar = logvar
        # mixing hyperparameter for loss_1 and loss_2 aggregation.
        self.mixing = mixing
        ...
        if full_gru_ode:
            if impute is False:
                ...
                self.gru_c = FullGRUODECell(
                    2 * input_size, hidden_size, bias=bias)
        ...
        assert solver in ["euler", "midpoint",
                          "dopri5"], "Solver must be either 'euler' or 'midpoint' or 'dopri5'."
    ...
    def ode_step(self, h, p, delta_t, current_time):
        ...
        if self.solver == "euler":
            ...
            solution, eval_times, eval_vals = odeint(self.gru_c, h, torch.tensor(
                [0, delta_t]), method=self.solver, options={"store_hist": self.store_hist})
            if self.store_hist:
                eval_ps = self.p_model(
                    torch.stack([ev[0] for ev in eval_vals]))
    ...
    def forward(self, times, time_ptr, X, M, obs_idx, delta_t, T, cov,
        ...
        h = torch.zeros(X.shape[0], self.args.hidden_dim).to(X)
        ...
        eval_times_total = torch.tensor(
            [], dtype=torch.float64, device=h.device)
        eval_vals_total = torch.tensor(
        ...
        for i, obs_time in enumerate(times):
            ...
            loss_2 = loss_2 + \
                compute_KL_loss(p_obs=p[i_obs], X_obs=X_obs,
                                M_obs=M_obs, logvar=self.logvar)
        ...
        if smoother:
            class_loss_vec += class_criterion(
                self.classification_model(h), labels).squeeze(1)
...
class GRUODEBayesSeq(torch.nn.Module):
    def __init__(self, input_size, hidden_size, p_hidden, prep_hidden, bias=True, cov_size=1, cov_hidden=1, classification_hidden=1, logvar=True, mixing=1, dropout_rate=0, obs_noise_std=1e-2, full_gru_ode=False):
        ...
        self.gru_bayes = SeqGRUBayes(input_size=input_size, hidden_size=hidden_size,
                                     prep_hidden=prep_hidden, p_hidden=p_hidden, bias=bias)
    ...
    def forward(self, times, time_ptr, Xpadded, Fpadded, X, M, lengths,
        ...
        for i, obs_time in enumerate(times):
            ...
            X_obs = pack_padded_sequence(
                Xpadded[start:end], L_obs, batch_first=True)
            F_obs = pack_padded_sequence(
                Fpadded[start:end], L_obs, batch_first=True)
            i_obs = obs_idx[start:end]
            ...
            loss_2 = loss_2 + \
                compute_KL_loss(
                    p_obs=p[i_obs], X_obs=Xf_batch, M_obs=Mf_batch, obs_noise_std=self.obs_noise_std)
...
class SeqGRUBayes(torch.nn.Module):
    ...
    def step_1feature(self, hidden, X_step, F_step):
        ...
        # TODO: try removing X_obs (they are included in error)
        gru_input = torch.stack(
            [X_step, mean, logvar, error], dim=1).unsqueeze(1)
