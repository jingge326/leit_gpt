...
from libs.pypots.forecasting.base import BaseForecaster
...
def sample_factor_u(tau_sparse_tensor, tau_ind, U, V, X, beta0=1):
    ...
    var_U_hyper = inv(
        np.eye(rank) + cov_mat(U, U_bar) + temp *
        beta0 * np.outer(U_bar, U_bar)
    )
...
def sample_factor_v(tau_sparse_tensor, tau_ind, U, V, X, beta0=1):
    ...
    var_V_hyper = inv(
        np.eye(rank) + cov_mat(V, V_bar) + temp *
        beta0 * np.outer(V_bar, V_bar)
    )
...
def sample_factor_x(tau_sparse_tensor, tau_ind, time_lags, U, V, X, A, Lambda_x):
    ...
    for t in range(dim3):
        ...
        X[t, :] = mvnrnd_pre(
            solve(var3[:, :, t], var4[:, t] + Nt + Qt), var3[:, :, t])
...
def _BTTF(
    ...
    for it in range(burn_iter + gibbs_iter):
        ...
        if it + 1 > burn_iter:
            ...
            tensor_new_plus += np.einsum("is, js, ts -> ijt",
                                         U, V, X0[-multi_step:, :])
...
def _BTTF_partial(
    ...
    for it in range(gibbs_iter):
        ...
        X0 = ar4cast(A_plus[:, :, it], X,
                     Sigma_plus[:, :, it], time_lags, multi_step)
        X_new_plus[:, :, it] = X0
        tensor_new_plus += np.einsum(
            "is, js, ts -> ijt", U_plus[:, :,
                                        it], V_plus[:, :, it], X0[-multi_step:, :]
        )
