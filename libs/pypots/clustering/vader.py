...
class _VaDER(nn.Module):
    ...
    def cluster(self, inputs):
        ...
        def func_to_apply(mu_t_, mu_, stddev_, phi_):
            ...
            return np.log(self.eps + phi_) + np.log(
                self.eps
                + multivariate_normal.pdf(mu_t_,
                                          mean=mu_, cov=np.diag(stddev_))
            )
    ...
    def forward(self, inputs, pretrain=False):
        ...
        unscaled_reconstruction_loss = cal_mse(
            X_reconstructed, X, missing_mask)
        reconstruction_loss = (
        ...
        sc_b = var_c.index_select(dim=0, index=ii)
        term2 = torch.reshape(
            st_b / (sc_b + self.eps), [batch_size,
                                       self.n_clusters, self.d_mu_stddev]
        )
        ...
        try:
            for epoch in range(self.epochs):
                ...
                if val_loader is not None:
                    ...
                    print(
                        f"epoch {epoch}: training loss {mean_train_loss:.4f}")
