...
from libs.pypots.clustering.base import BaseNNClusterer
from libs.pypots.data.dataset_for_grud import DatasetForGRUD
from libs.pypots.utils.metrics import cal_mse
...
class MultiRNNCell(nn.Module):
    ...
    def forward(self, inputs):
        ...
        output_collector = torch.empty(
            (bz, n_steps, self.d_input), device=self.device)
        if self.cell_type == "LSTM":
            # TODO: cell states should have different shapes
            cell_states = torch.zeros(
                (self.d_input, self.d_hidden), device=self.device)
            for step in range(n_steps):
                ...
                imputed_x = (
                    missing_mask[:, step] * x +
                    (1 - missing_mask[:, step]) * estimation
                )
...
class Generator(nn.Module):
    def __init__(self, n_layers, n_features, d_hidden, cell_type, device):
        super().__init__()
        self.f_rnn = MultiRNNCell(
            cell_type, n_layers, n_features, d_hidden, device)
        self.b_rnn = MultiRNNCell(
            cell_type, n_layers, n_features, d_hidden, device)

    def forward(self, inputs):
        f_outputs, f_final_hidden_state = self.f_rnn(inputs)
        b_outputs, b_final_hidden_state = self.b_rnn(inputs)
        # reverse the output of the backward rnn
...
class Discriminator(nn.Module):
    ...
    def forward(self, inputs):
        ...
        hidden_state_collector = torch.empty(
            (bz, n_steps, 32), device=self.device)
        if self.cell_type == "LSTM":
            cell_states = torch.zeros(
                (self.d_input, self.d_hidden), device=self.device)
            for step in range(n_steps):
                ...
                for i, rnn_cell in enumerate(self.rnn_cell_module_list):
                    if i == 0:
                        ...
                        hidden_state = rnn_cell(
                            hidden_states[i - 1], hidden_states[i])
                    hidden_states[i] = hidden_state
                hidden_state_collector[:, step, :] = hidden_state
...
class _CRLI(nn.Module):
    ...
    def cluster(self, inputs, training_object="generator"):
        ...
        imputation, imputed_X, generator_fb_hidden_states = self.generator(
            inputs)
    ...
    def forward(self, inputs, training_object="generator"):
        ...
        if training_object == "discriminator":
            ...
            HTH = torch.matmul(inputs["fcn_latent"],
                               inputs["fcn_latent"].permute(1, 0))
            term_F = torch.nn.init.orthogonal_(
                torch.randn(batch_size, self.n_clusters, device=self.device), gain=1
            )
            FTHTHF = torch.matmul(torch.matmul(
                term_F.permute(1, 0), HTH), term_F)
...
class CRLI(BaseNNClusterer):
    def __init__(
        ...
        self.logger = {"training_loss_generator": [],
    ...
    def _train_model(self, training_loader, val_loader=None):
            ...
            for epoch in range(self.epochs):
                ...
                for idx, data in enumerate(training_loader):
                    ...
                    for _ in range(self.D_steps):
                        ...
                        epoch_train_loss_D_collector.append(
                            results["l_disc"].item())
                    ...
                    for _ in range(self.G_steps):
                        ...
                        epoch_train_loss_G_collector.append(
                            results["l_gene"].item())
                ...
                self.logger["training_loss_generator"].append(
                    mean_train_G_loss)
                self.logger["training_loss_discriminator"].append(
                    mean_train_D_loss)
                print(
                    f"epoch {epoch}: "
        ...
        if np.equal(self.best_loss, float("inf")):
            raise ValueError(
                "Something is wrong. best_loss is Nan after training.")
