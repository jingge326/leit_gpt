...
from libs.pypots.classification.base import BaseNNClassifier
    ...
    def message(
        ...
        if edge_weights is not None:
            if use_beta:
                ...
                index_top_edges = torch.argsort(
                    all_edge_weights, descending=True)[:K]
        ...
        decompose = False
        if not decompose:
            out = F.relu(self.lin_value(x_i)).view(-1,
...
class _Raindrop(nn.Module):
    def __init__(
        ...
        self.encoder = nn.Linear(
            n_features * self.d_ob, n_features * self.d_ob)
        d_pe = 16
        ...
        self.adj = torch.ones(
            [self.n_features, self.n_features], device=self.device)
    ...
    def classify(self, inputs):
        ...
        alpha_all = torch.zeros(
            [edge_index.shape[1], batch_size], device=self.device)
    ...
    def forward(self, inputs):
        ...
        classification_loss = F.nll_loss(
            torch.log(prediction), inputs["label"])
...
class Raindrop(BaseNNClassifier):
    ...
    def fit(self, train_X, train_y, val_X=None, val_y=None):
        ...
        val_X, val_y = self.check_input(
            self.n_steps, self.n_features, val_X, val_y)
    ...
    def classify(self, X):
        ...
        with torch.no_grad():
            for idx, data in enumerate(test_loader):
                ...
                lengths = torch.tensor([n_steps] * bz, dtype=torch.float)
                times = torch.tensor(
                    range(n_steps), dtype=torch.float).repeat(bz, 1)
