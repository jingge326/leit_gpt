...
from .functional.causalconv import causal_conv, causal_fftconv
...
class KernelNet(torch.nn.Module):
    def __init__(
        ...
        self.kernel_net = torch.nn.Sequential(
            ...
            torch.nn.Dropout(
                p=weight_dropout) if w_dp else torch.nn.Identity(),
        )
                ...
                if (
                    ...
                    if m.bias is not None:
                        ...
                        if net_layer == 1:
                            ...
                            range = torch.linspace(-1.0,
                                                   1.0, steps=m.weight.shape[0])
...
class CKConv(torch.nn.Module):
    def __init__(
        ...
        self.register_buffer("conv_kernel", torch.zeros(
            in_channels), persistent=False)
    ...
    def forward(self, x):
        ...
        assert not torch.isnan(conv_kernel).any()
        ...
        if self.sigma is not None:
            ...
            smoothing_ker = torch.Tensor(
                smoothing_ker).cuda().unsqueeze(0).unsqueeze(0)
    ...
    def handle_rel_positions(self, x):
        ...
        if self.rel_positions is None:
            ...
            if self.train_length.item() > x.shape[-1]:
                ...
                self.sr_change = 1 / \
                    round(x.shape[-1] / self.train_length.item())
