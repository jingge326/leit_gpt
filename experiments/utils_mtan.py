...
def evaluate(dim, rec, dec, test_loader, args, num_sample=10, device="cuda"):
    ...
    with torch.no_grad():
        for test_batch in test_loader:
            ...
            out = rec(
                torch.cat((subsampled_data, subsampled_mask), 2), subsampled_tp)
            qz0_mean, qz0_logvar = (
                out[:, :, : args.latent_dim],
                out[:, :, args.latent_dim:],
            )
            ...
            time_steps = (
                observed_tp[None, :, :].repeat(
                    num_sample, 1, 1).view(-1, seqlen)
            )
            pred_x = dec(z0, time_steps)
            pred_x = pred_x.view(
                num_sample, -1, pred_x.shape[1], pred_x.shape[2])
            pred_x = pred_x.mean(0)
            mse += mean_squared_error(observed_data,
                                      pred_x, observed_mask) * batch
            test_n += batch
    return mse / test_n
...
def compute_losses(observed_data, observed_mask, qz0_mean, qz0_logvar, pred_x, args):
    obsrv_std = args.obsrv_std  # default 0.1
    noise_std_ = torch.zeros(pred_x.size()).to(
        observed_data.device) + obsrv_std
    noise_logvar = 2. * torch.log(noise_std_).to(observed_data.device)
    logpx = log_normal_pdf(observed_data, pred_x, noise_logvar,
                           observed_mask).sum(-1).sum(-1)
    pz0_mean = pz0_logvar = torch.zeros(
        qz0_mean.size()).to(observed_data.device)
...
def evaluate_classifier(model, test_loader, dec=None, args=None, classifier=None,
                        dim=41, device='cuda:3', reconst=False, num_sample=1):
    pred = []
    true = []
    test_loss = 0
    for test_batch, label in test_loader:
        ...
        with torch.no_grad():
            ...
            if args.classify_pertp:
                ...
                label = label.repeat(num_sample, *label.shape)
...
def get_toy_data(args):
    ...
    obs_values, ground_truth, obs_times = irregularly_sampled_data_gen(
        args.n_samples, args.length)
    obs_times = np.array(obs_times).reshape(args.n_samples, -1)
    obs_values = np.array(obs_values)
    combined_obs_values = np.zeros((args.n_samples, dim, obs_times.shape[-1]))
    mask = np.zeros((args.n_samples, dim, obs_times.shape[-1]))
    for i in range(dim):
