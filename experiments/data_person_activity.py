...
class PersonActivity(object):
    ...
    def __init__(self, root, download=False,
        ...
        self.data = torch.load(os.path.join(
            self.processed_folder, self.data_file), map_location='cpu')
                ...
                with open(os.path.join(dirname, txtfile)) as f:
                    ...
                    for l in lines:
                        ...
                        value_vec = torch.Tensor(
                            (float(val1), float(val2), float(val3))).to(self.device)
                        time = float(time)
                        ...
                        if cur_record_id != record_id:
                            ...
                            vals = [torch.zeros(
                                len(self.tag_ids), 3).to(self.device)]
                            mask = [torch.zeros(
                                len(self.tag_ids), 3).to(self.device)]
                            nobs = [torch.zeros(
                                len(self.tag_ids)).to(self.device)]
                            labels = [torch.zeros(
                                len(self.label_names)).to(self.device)]
...
def collate_fn_activity(batch, args, device=torch.device('cpu'), data_type='train'):
    '''
    ...
    data_dict = {
        'data': combined_vals.to(device),  # (batch, sequence, dim=12)
        'times': combined_tt.to(device),  # (batch, sequence)
        'mask': combined_mask.to(device),  # (batch, sequence, dim=12)
        'truth': combined_labels.to(device)  # (batch, sequence, dim=11)
    }
